...
# ::::::::::::::::::::::::::::::::::::::::::
  ...
  then
    ...
    then
      ...
      do
        ...
        then
          ...
          trap \
          "RUNFILE_SKIP_SUBTASKS=1 RUNFILE_TRAP=${trap_sig} run ${make_args[*]}" \
          "${trap_sig}"
        fi
      done
    elif [[ "${RUNFILE_TRAP}" != '*' ]]
    then
      ...
      # A trap is being triggered due to these two env vars being set.
      # Unsetting them now that they've been read allows traps to call
      # 'run' tasks normally without these vars continuing to be set:
      unset RUNFILE_SKIP_SUBTASKS
      unset RUNFILE_TRAP
    fi
  fi
  if (( ${#named_args[@]} ))
